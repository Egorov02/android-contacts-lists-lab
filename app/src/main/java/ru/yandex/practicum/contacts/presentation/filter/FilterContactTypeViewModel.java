...
public class FilterContactTypeViewModel extends BaseBottomSheetViewModel {
    ...
    private void updateFilterContactTypes() {
        String [] arrayTypes = ContactType.getContactTypes();// создайте массив строк. Для инициализации используйте метод ContactType.getContactTypes(),
        // который должен возвращать список всех доступных источников контактов

        ArrayList<FilterContactTypeUi> filterContactTypeUiList = new ArrayList<>();// создайте список типа FilterContactTypeUi и заполните его с помощью цикла forEach
        filterContactTypeUiList.add(createAllSelectedItem(arrayTypes));
        for (String arrayType : arrayTypes) {
            filterContactTypeUiList.add(createFilterContactType(arrayType));
